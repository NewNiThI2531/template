...
if($key==""){
  $sql = " SELECT * FROM `database_blog` WHERE `bg_page` = 12 AND `bg_status`=0 AND `bg_type` = 2  LIMIT $limit OFFSET  $offset";  
}else{
...
if (isset($_REQUEST['method']) && $_REQUEST['method']=="content-num") {
  $key = $conn->real_escape_string($_POST['key']);
  if($key == ""){
    $sql = " SELECT * FROM `database_blog` WHERE `bg_page` = 12 AND `bg_status`=0 AND `bg_type` = 2  ";  
