...
if(isset($_POST['aid1'])){
      ...
      $sql = "UPDATE `Database_contact` 

      SET `ct_headtext`     = '$title1',
          `ct_headtextEN`  = '$title2'  
      WHERE `ct_id` = '$aid'";
...
if(isset($_POST['aid2'])){
  ...
  $sql = "UPDATE `Database_contact` 
  SET `ct_headtext`     = '$title3',
      `ct_headtextEN`  = '$title4',  
      `ct_text`     = '$title5',
      `ct_textEN`  = '$title6'    
       WHERE `ct_id` = '$aid2'";
...
if(isset($_POST['aid4'])){
  ...
  $sql = "UPDATE `Database_contact` 
  SET `ct_headtext`     = '$title41',
      `ct_headtextEN`  = '$title42',  
      `ct_text`     = '$title43',
      `ct_textEN`  = '$title44'    
       WHERE `ct_id` = '$aid4'";
...
if ($query) {
  ...
  if(isset($_POST['aid41'])){
    ...
    $sql = "UPDATE `Database_contact` 
    SET `ct_headtext`     = '$title45',
        `ct_headtextEN`  = '$title46',  
        `ct_text`     = '$title47',
        `ct_textEN`  = '$title48'    
         WHERE `ct_id` = '$aid41'";
...
if(isset($_POST['aid13'])){
  $title17 = htmlspecialchars($_POST['content12_TH'], ENT_QUOTES);
  $title18 = htmlspecialchars($_POST['content12_EN'], ENT_QUOTES);
...
if(isset($_POST['aid5'])){
  ...
  $sql2 = "UPDATE `Database_contact` 

  SET `ct_headtext`     = '$title11',
      `ct_headtextEN`  = '$title12'  
  WHERE `ct_id` = '$aid5'";
...
echo json_encode($data);
 ...
 exit();
    ...
    if($chk != ''){
      $sql = "UPDATE `Database_contact` SET `ct_status` = '$chk' WHERE `Database_contact`.`ct_id` = '$id6'";
  echo $sql  ;
      $query = $conn->query($sql);
      if($query){
        $data = ['res_code' => '00' , 'res_text' => 'บันทึกข้อมูลเรียบร้อย'];
...
if(isset($_POST['banid'])){
    ...
    if($chk != ''){
      $sql = "UPDATE `Database_contact` SET `ct_status` = '$chk' WHERE `Database_contact`.`ct_id` = '$id'";
      //  print_r($sql ) ;
      $query = $conn->query($sql);
      if($query){
        $data = ['res_code' => '00' , 'res_text' => 'บันทึกข้อมูลเรียบร้อย'];
          ...
          if(move_uploaded_file($_FILES["fileuplad"]["tmp_name"], $path_upload)){
            ...
            if ($id != '') {
              // $sql = "INSERT INTO `Database_setting` ( `set_image`, `Set_Type`) VALUES ('โปรโมชัน', '$path_update', '1')";
               $sql = "UPDATE `Database_contact` SET `ct_image` = '$path_update' WHERE `ct_id` = '$id'";
