  ...
  var save_aticle = document.getElementsByClassName('fron_IN');
    var validation = Array.prototype.filter.call(save_aticle, function(form) {
      form.addEventListener('submit', function(event) {
        ...
        loadingShow();
                $.ajax({
                    url: 'view/contacus/calldataA.php',
                    method : "POST",
                    data   : data,
                    dataType: "json",
                    contentType: false,
    ...
    var validation = Array.prototype.filter.call(save_book, function(form) {
      ...
      $('body').on('click', '.btn-edit', function () {
        ...
        $.ajax({
          ...
          success :function(data){
            if(data.res_code=='00'){
              $('#aid').val(data.res_data.ct_id);
              $('#title1_TH').val(data.res_data.ct_headtext);
              $('#title1_EN').val(data.res_data.ct_headtextEN);
              $('#title2_TH').val(data.res_data.ct_headtext);
              $('#title2_EN').val(data.res_data.ct_headtextEN);
              $('#title5_TH').val(data.res_data.ct_headtext);
              $('#title5_EN').val(data.res_data.ct_headtextEN);
              $('#title4_TH').val(data.res_data.ct_headtext);
              $('#title4_EN').val(data.res_data.ct_headtextEN);
              $('#title41_TH').val(data.res_data.ct_headtext);
              $('#title41_EN').val(data.res_data.ct_headtextEN);
  
              $('#content42_TH').val(data.res_data.ct_text);
              $('#content42_EN').val(data.res_data.ct_textEN);
              $('#content41_TH').val(data.res_data.ct_text);
              $('#content41_EN').val(data.res_data.ct_textEN);
              $('#content_TH').val(data.res_data.ct_text);
              $('#content_EN').val(data.res_data.ct_textEN);
              $('#previewImg').attr("src", '../'+data.res_data.ct_image);
              $('#modal-edit-article').modal('show')
