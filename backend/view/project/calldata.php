...
if($_REQUEST['method'] == 'get_article'){
  ...
  $offset = $_GET['offset'];
  $limit = $_GET['limit'];
  $sql = "SELECT * FROM `Database_Project` WHERE pro_type = '2'";
  ...
  if($query->num_rows > 0){
    while ($res = $query->fetch_assoc()) {
        ...
        <div class="card-body">
          <div class="row">
            <div class="col-2 card-detail">
              <div class="img-card" style="background: url(../'.$res["pro_img"].')">
              </div>
            </div>
            <div class="col-8
             card-detail">
            ...
            <div class="col-2" style="text-align:center;align-self: center;">
                      ...
                      <input class="form-check-input cheak" type="checkbox" style="width: 50px;" data-id=" ' . $res["pro_id"] . '"
                      ' . ($res["pro_status"] == "0" ? "checked" : "") . ' >                      </div>
                </div>
          </div>
        </div>
      </div>
      ';
      $res["data"] = $html;
      $res["manage"] = '<button class="btn btn-success btn-pen" data-id="'.$res["pro_id"].'">แก้ไข</button>
                        <button class="btn btn-danger btn-delete" data-id="'.$res["pro_id"].'">ลบ</button>
                       ';
      $output[] = $res;
    }
    $data = [
    ...
    $sql = "SELECT * FROM `Database_Project` WHERE `pro_id` = '$id'";
    $query = $conn->query($sql);
    if($query->num_rows > 0){
      while ($res = $query->fetch_assoc()) {
        $output = $res;
  ...
  if ($id != '') {
    $sql = "UPDATE `Database_Project` SET `pro_headtext` = '$title', `pro_text` = '$content' , `pro_headtextEN` = '$title_en', `pro_textEN` = '$content_en' WHERE `pro_id` = '$id'";
    ...
    $sql = "INSERT INTO `` (`pro_headtext`, `pro_text`, `pro_headtextEN`, `pro_textEN`)
  ...
  $id = htmlspecialchars($_POST['id'], ENT_QUOTES);
  $sql = "UPDATE `Database_Project` SET `pro_status` = '2' WHERE `Database_Project`.`pro_id` = '$id'";
  ...
  $image = "SELECT pro_img FROM Database_Project WHERE pro_id = '$id'";
  $query1 = $conn->query($image);
  $after = mysqli_fetch_assoc($query1);


  $sql = "DELETE FROM Database_Project WHERE `Database_Project`.`pro_id` = '$id'";
  $query = $conn->query($sql);
  if($query){
    $data = ['res_code' => '00' , 'res_text' => 'ลบบทความเรียบร้อย'];
    unlink("../../../".$after['pro_img']);
  ...
  if($chk != ''){
    $sql = "UPDATE `Database_Project` SET `pro_status` = '$chk' WHERE `Database_Project`.`pro_id` = '$id'";
  ...
  if ($id != '') {
    ...
    $sql = "INSERT INTO `Database_Project` (`pro_headtext`, `pro_text`, `pro_headtextEN`, `pro_textEN`)
      VALUES ('$title', '$content', '$title_en', '$content_en')";
    ...
    if(in_array($fileType, $allowTypes)){
      $path_upload = $path.$time.'.'.$fileType;
        ...
        if(move_uploaded_file($_FILES["fileuplad"]["tmp_name"], $path_upload)){
          ...
          if ($id == '') {
            $sql = "INSERT INTO `Database_Project` ( `pro_img`, `pro_type`) VALUES ('โปรโมชัน', '$path_update', '1')";
          }else{
            $sql = "UPDATE `Database_Project` SET `pro_img` = '$path_update' WHERE `pro_id` = '$id'";
          }
          $query = $conn->query($sql);
          $data = ['res_code' => '00' , 'res_text' => 'บันทึกข้อมูลเรียบร้อย'];
        }
  ...
  if($chk != ''){
    echo $sql = "UPDATE `Database_Project` SET `pro_status` = '$chk' WHERE `Database_Project`.`pro_id` = '$id'";
