...
if($_REQUEST['method'] == 'get_article'){
    $offset = $_GET['offset'];
    $limit = $_GET['limit'];
    $sql = "SELECT * FROM `Database_BOARD` WHERE BOARD_Page = '6' AND board_typetwo='3'";
    ...
    if($query->num_rows > 0){
      while ($res = $query->fetch_assoc()) {
        ...
        <div class="card-body">
          ...
          <div class="col-3 p-0" >
                  <div class="img-card" style="background: url(../../'.$res["BOARD_image"].');  height:160px;">
                  </div>
                </div>
                ...
                <div class="col-2" style="text-align:center; ">
                      ...
                      <input class="form-check-input cheak" type="checkbox" data-id=" ' . $res["BOARD_id"] . '"
                      ' . ($res["BOARD_Status"] == "0" ? "checked" : "") . ' >                      </div>
                </div>
        ...
        ';
        $res["data"] = $html; 
        $res["manage"] = '<button class="btn btn-success btn-pen" data-path="../../'.$res["BOARD_image"].'" data-id="'.$res["BOARD_id"].'">แก้ไข</button>
                          <button class="btn btn-danger btn-delete" data-path="../../'.$res["BOARD_image"].'" data-id="'.$res["BOARD_id"].'">ลบ</button>
                         ';
        $output[] = $res;
      }
      $data = [
  ...
  if ($id != '') {
    $sql = "UPDATE `Database_BOARD` SET `BOARD_Headtext` = '$title', `BOARD_Text` = '$content' , `BOARDHeadtext_EN` = '$title_en', `BOARDtext_EN` = '$content_en' WHERE `BOARD_id` = '$id'";
  ...
  $image = "SELECT BOARD_image FROM Database_BOARD WHERE BOARD_id = '$id'";
  $query1 = $conn->query($image);
  $after = mysqli_fetch_assoc($query1);


  $sql = "DELETE FROM Database_BOARD WHERE `Database_BOARD`.`BOARD_id` = '$id'";
  $query = $conn->query($sql);
  if($query){
    $data = ['res_code' => '00' , 'res_text' => 'ลบบทความเรียบร้อย'];
    unlink("../../../".$after['BOARD_image']);
  ...
  if ($id != '') {
     $sql = "UPDATE `Database_BOARD` SET `BOARD_Type` = '$blog_blType' , `BOARD_Headtext` = '$title', `BOARD_Text` = '$content' , `BOARDHeadtext_EN` = '$title_en', `BOARDtext_EN` = '$content_en' WHERE `BOARD_id` = '$id'";
    ...
    if(in_array($fileType, $allowTypes)){
      $path_upload = $path.$time.'.'.$fileType;
        ...
        if(move_uploaded_file($_FILES["fileuplad"]["tmp_name"], $path_upload)){
          ...
          if ($id == '') {
            $sql = "INSERT INTO `Database_BOARD` ( `BOARD_image`, `board_typetwo`) VALUES ('โปรโมชัน', '$path_update', '1')";
          }else{
