...
if($_REQUEST['method'] == 'get_article'){
    ...
    if($query->num_rows > 0){
      while ($res = $query->fetch_assoc()) {
        ...
        ';
        $res["data"] = $html;
        $res["manage"] = '<button class="btn btn-success btn-edit" data-id="'.$res["commer_id"].'">แก้ไข</button>
                          <button class="btn btn-danger btn-delete" data-id="'.$res["commer_id"].'">ลบ</button>
                         ';
        $output[] = $res;
      }
      $data = [
    ...
    $sql = "SELECT * FROM `Database_COMMERCIAL` WHERE `commer_id` = '$id'";
    $query = $conn->query($sql);
    if($query->num_rows > 0){
      while ($res = $query->fetch_assoc()) {
        $output = $res;
  ...
  if ($id != '') {
    $sql = "UPDATE `Database_COMMERCIAL` SET `commer_headtext` = '$title', `commer_text` = '$content' , `commer_headtextEN` = '$title_en', `commer_textEN` = '$content_en' WHERE `commer_id ` = '$id'";
    ...
    $sql = "INSERT INTO `Database_COMMERCIAL` (`commer_headtext`, `commer_text`, `commer_headtextEN`, `commer_textEN`)
  ...
  $id = htmlspecialchars($_POST['id'], ENT_QUOTES);
  $sql = "UPDATE `Database_COMMERCIAL` SET `commer_status` = '2' WHERE `commer_text`.`commer_id ` = '$id'";
  ...
  if($chk != ''){
    $sql = "UPDATE `Database_COMMERCIAL` SET `commer_status` = '$chk' WHERE `Database_COMMERCIAL`.`commer_id` = '$id'";
    ...
    if(in_array($fileType, $allowTypes)){
      $path_upload = $path.$time.'.'.$fileType;
        ...
        if(move_uploaded_file($_FILES["fileuplad"]["tmp_name"], $path_upload)){
          ...
          if ($id == '') {
            $sql = "INSERT INTO `Database_COMMERCIAL` ( `commer_image`, `commer_type`) VALUES ('โปรโมชัน', '$path_update', '1')";
          }else{
            $sql = "UPDATE `Database_setting` SET `commer_image` = '$path_update' WHERE `commer_id` = '$id'";
          }
          $query = $conn->query($sql);
          $data = ['res_code' => '00' , 'res_text' => 'บันทึกข้อมูลเรียบร้อย'];
        }
