...
if($_REQUEST['method'] == 'get_article'){
    $offset = $_GET['offset'];
    $limit = $_GET['limit'];
    $sql = "SELECT * FROM `database_blog` WHERE bg_page = '12' AND bg_type='2'";
    ...
    if($query->num_rows > 0){
      while ($res = $query->fetch_assoc()) {
        ...
        <div class="card-body">
          ...
          <div class="col-3 p-0" >
                  <div class="img-card" style="background: url(../../'.$res["bg_image"].');  height:160px;">
                  </div>
                </div>
                ...
                Type : '.$res["blog_blType"].'
               
                </div>
                
                <div class="col-2" style="text-align:center; ">
                      <div class="form-check form-switch text-center">
                      <input class="form-check-input cheak" type="checkbox" data-id=" ' . $res["bg_id"] . '"
                      ' . ($res["bg_status"] == "0" ? "checked" : "") . ' >                      </div>
                </div>
        ...
        ';
        $res["data"] = $html; 
        $res["manage"] = '<button class="btn btn-success btn-pen" data-path="../../'.$res["bg_image"].'" data-id="'.$res["bg_id"].'">แก้ไข</button>
                          <button class="btn btn-danger btn-delete" data-path="../../'.$res["bg_image"].'" data-id="'.$res["bg_id"].'">ลบ</button>
                         ';
        $output[] = $res;
      }
      $data = [
    ...
    $sql = "SELECT * FROM `database_blog` WHERE `bg_id` = '$id'";
    $query = $conn->query($sql);
    if($query->num_rows > 0){
      while ($res = $query->fetch_assoc()) {
        $output = $res;
  ...
  if ($id != '') {
    $sql = "UPDATE `database_blog` SET `bg_text` = '$title', `bg_content` = '$content' , `bgtext_EN` = '$title_en', `bgcontent_EN` = '$content_en' WHERE `bg_id` = '$id'";
    ...
    $sql = "INSERT INTO `database_blog` (`bg_text`, `bg_content`, `bgtext_EN`, `bgcontent_EN`)
  ...
  $id = htmlspecialchars($_POST['id'], ENT_QUOTES);
  $sql = "UPDATE `database_blog` SET `bg_status` = '2' WHERE `database_blog`.`bg_id` = '$id'";
  ...
  $image = "SELECT bg_image FROM database_blog WHERE bg_id = '$id'";
  $query1 = $conn->query($image);
  $after = mysqli_fetch_assoc($query1);


  $sql = "DELETE FROM database_blog WHERE `database_blog`.`bg_id` = '$id'";
  $query = $conn->query($sql);
  if($query){
    $data = ['res_code' => '00' , 'res_text' => 'ลบบทความเรียบร้อย'];
    unlink("../../../".$after['bg_image']);
  ...
  if($chk != ''){
    $sql = "UPDATE `database_blog` SET `bg_status` = '$chk' WHERE `database_blog`.`bg_id` = '$id'";
  ...
  $title_en = $conn->real_escape_string($_POST['title_en']);
  $content_en = $conn->real_escape_string($_POST['content_en']);
  $blog_blType = $conn->real_escape_string($_POST['sl']);
  if ($id != '') {
     $sql = "UPDATE `database_blog` SET `blog_blType` = '$blog_blType' , `bg_text` = '$title', `bg_content` = '$content' , `bgtext_EN` = '$title_en', `bgcontent_EN` = '$content_en' WHERE `bg_id` = '$id'";
    ...
    if(in_array($fileType, $allowTypes)){
      $path_upload = $path.$time.'.'.$fileType;
        ...
        if(move_uploaded_file($_FILES["fileuplad"]["tmp_name"], $path_upload)){
          ...
          if ($id == '') {
            $sql = "INSERT INTO `database_blog` ( `bg_image`, `bg_type`) VALUES ('โปรโมชัน', '$path_update', '1')";
          }else{
            $sql = "UPDATE `database_blog` SET `bg_image` = '$path_update' WHERE `bg_id` = '$id'";
          }
          $query = $conn->query($sql);
          $data = ['res_code' => '00' , 'res_text' => 'บันทึกข้อมูลเรียบร้อย'];
        }
