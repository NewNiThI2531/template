  ...
  var save_aticle = document.getElementsByClassName('fron_IN');
    var validation = Array.prototype.filter.call(save_aticle, function(form) {
      form.addEventListener('submit', function(event) {
        ...
        loadingShow();
                $.ajax({
                    url: 'view/ourbrand/calldataA.php',
                    method : "POST",
                    data   : data,
                    dataType: "json",
                    contentType: false,
...
// ------------------------------------------------------------
      ...
      function ajaxRequest(params) {
        var url = 'view/ourbrand/calldataA.php'
        ...
        $('body').on('click', '.btn-pen', function () {
            ...
            $.ajax({
              ...
              success : function(data){
                // console.log(data);
                if(data.res_code=='00'){
                  $('#aid').val(data.res_data.news_id);
                  $('#title').val(data.res_data.news_headtext);
                  $('#content').val(data.res_data.news_text);
                  $('#title_en').val(data.res_data.news_headtextEN);
                  $('#content_en').val(data.res_data.news_textEN);
                  $('#modal-edit-article').modal('show')
